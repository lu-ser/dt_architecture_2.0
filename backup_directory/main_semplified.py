...
async def quick_test():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            user_data = {'username': f'test_user_{int(datetime.utcnow().timestamp())}', 'email': f'test_{int(datetime.utcnow().timestamp())}@example.com', 'password': 'TestPass123!', 'first_name': 'Test', 'last_name': 'User', 'plan': 'free'}
            ...
            if response.status_code == 200:
                ...
                if reg_data.get('status') == 'registration_complete' and 'tokens' in reg_data:
                    ...
                    response = await client.get(f'{base_url}/api/v1/secure/digital-twins/security/status', headers=headers)
                    if response.status_code == 200:
