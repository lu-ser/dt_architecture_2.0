...
from src.utils.exceptions import RegistryError, EntityNotFoundError, EntityAlreadyExistsError, RegistryConnectionError, StorageError
...
class RegistryMetrics:
    ...
    def record_operation(self, success: bool, response_time: float) -> None:
        ...
        self.average_response_time = (self.average_response_time * (self.total_operations - 1) + response_time) / self.total_operations
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'total_operations': self.total_operations, 'successful_operations': self.successful_operations, 'failed_operations': self.failed_operations, 'average_response_time': self.average_response_time, 'cache_hits': self.cache_hits, 'cache_misses': self.cache_misses, 'cache_hit_ratio': self.get_cache_hit_ratio(), 'success_ratio': self.get_success_ratio(), 'last_health_check': self.last_health_check.isoformat() if self.last_health_check else None, 'entity_count': self.entity_count}
...
class BaseCache:
    ...
    def set(self, key: str, value: Any) -> None:
        ...
        self._cache[key] = {'value': value, 'created_at': datetime.now(timezone.utc)}
        self._access_times[key] = datetime.now(timezone.utc)
...
class AbstractRegistry(IRegistry[T], ABC):
    ...
    def __init__(self, entity_type: Type[T], storage_adapter: IStorageAdapter[T], cache_enabled: bool=True, cache_size: int=1000, cache_ttl: int=300):
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        metrics_dict.update({'registry_name': self.registry_name, 'entity_type': self.entity_type.__name__, 'cache_enabled': self.cache_enabled, 'cache_size': self.cache.size() if self.cache else 0, 'connected': self._connected})
        return metrics_dict
    ...
    def __repr__(self) -> str:
        return f'{self.__class__.__name__}(entity_type={self.entity_type.__name__}, connected={self._connected}, cache_enabled={self.cache_enabled})'

class RegistryFactory:

    @staticmethod
    def create_registry(entity_type: Type[T], storage_adapter: IStorageAdapter[T], registry_class: Optional[Type[AbstractRegistry]]=None, **kwargs) -> AbstractRegistry[T]:
        if registry_class is None:
            registry_class = AbstractRegistry
        return registry_class(entity_type=entity_type, storage_adapter=storage_adapter, **kwargs)