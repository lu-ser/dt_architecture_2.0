...
class RedisCache:
    ...
    async def connect(self) -> None:
        if self._connected:
            return
        try:
            redis_config = {'host': self.config.get('redis.host', 'localhost'), 'port': self.config.get('redis.port', 6379), 'db': self.config.get('redis.database', 0), 'password': self.config.get('redis.password'), 'socket_timeout': self.config.get('redis.connection_timeout', 5), 'socket_connect_timeout': 5, 'retry_on_timeout': self.config.get('redis.retry_on_timeout', True), 'decode_responses': True, 'max_connections': self.config.get('redis.max_connections', 20)}
            redis_config = {k: v for k, v in redis_config.items() if v is not None}
            self._redis = aioredis.from_url(f"redis://{redis_config['host']}:{redis_config['port']}/{redis_config['db']}", **{k: v for k, v in redis_config.items() if k not in ['host', 'port', 'db']})
