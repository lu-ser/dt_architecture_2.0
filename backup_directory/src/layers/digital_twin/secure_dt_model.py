...
class SecureDigitalTwin(StandardDigitalTwin):
    ...
    def __init__(self, twin_id: UUID, configuration: DigitalTwinConfiguration, metadata: BaseMetadata, owner_id: UUID, tenant_id: UUID, models: Optional[List[TwinModel]]=None):
        ...
        self.access_permissions: Dict[UUID, Set[str]] = {owner_id: {'read', 'write', 'execute', 'admin', 'manage_access'}}
    ...
    def to_dict(self, include_security_details: bool=False) -> Dict[str, Any]:
        ...
        base_dict.update({'owner_id': str(self.owner_id), 'tenant_id': str(self.tenant_id), 'is_public': self.is_public, 'shared_with_tenant': self.shared_with_tenant, 'authorized_users_count': len(self.authorized_users), 'last_accessed_by': str(self.last_accessed_by) if self.last_accessed_by else None})
        if include_security_details:
            base_dict.update({'authorized_users': {str(uid): level.value for uid, level in self.authorized_users.items()}, 'access_permissions': {str(uid): list(perms) for uid, perms in self.access_permissions.items()}, 'recent_access_log': self.access_log[-10:], 'dt_identity': self.dt_identity.to_dict() if self.dt_identity else None})
        return base_dict