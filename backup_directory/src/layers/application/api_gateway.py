...
class APIGateway:
    ...
    async def execute_twin_capability(self, twin_id: UUID, capability: str, input_data: Dict[str, Any], execution_config: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        try:
            ...
            return {'twin_id': str(twin_id), 'capability': capability, 'result': result, 'executed_at': datetime.now(timezone.utc).isoformat()}
        except Exception as e:
    ...
    async def discover_entities(self, entity_type: RequestType, criteria: Dict[str, Any]) -> List[Dict[str, Any]]:
        try:
