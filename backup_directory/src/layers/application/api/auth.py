...
class UserRegistration(BaseModel):
    ...
    class Config:
        schema_extra = {'example': {'username': 'john_doe', 'email': 'john@example.com', 'password': 'StrongPass123!', 'first_name': 'John', 'last_name': 'Doe', 'company_name': 'Acme Corp', 'plan': 'pro'}}
...
class UserLogin(BaseModel):
    ...
    class Config:
        schema_extra = {'example': {'username': 'john_doe', 'password': 'StrongPass123!'}}
...
async def login_user(login_data: UserLogin, request: Request, registration_service: UserRegistrationService=Depends(get_registration_service)) -> Dict[str, Any]:
    try:
        ...
        return {'access_token': token_pair.access_token, 'refresh_token': token_pair.refresh_token, 'token_type': token_pair.token_type, 'expires_in': token_pair.expires_in, 'user': user.to_dict(), 'tenant': tenant_info}
...
async def refresh_token(refresh_token: str, registration_service: UserRegistrationService=Depends(get_registration_service)) -> Dict[str, Any]:
    try:
        jwt_provider = registration_service.jwt_provider
        new_token_pair = await jwt_provider.refresh_token(refresh_token)
        return {'access_token': new_token_pair.access_token, 'refresh_token': new_token_pair.refresh_token, 'token_type': new_token_pair.token_type, 'expires_in': new_token_pair.expires_in}
    except AuthenticationError as e:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=str(e))

@router.post('/logout', summary='Logout User')
async def logout_user(auth_context: AuthContext=Depends(get_auth_context), registration_service: UserRegistrationService=Depends(get_registration_service)) -> Dict[str, str]:
    try:
        jwt_provider = registration_service.jwt_provider
...
async def invite_user_to_tenant(invite_data: InviteUser, auth_context: AuthContext=Depends(get_auth_context), registration_service: UserRegistrationService=Depends(get_registration_service)) -> Dict[str, Any]:
    try:
        ...
        result = await registration_service.invite_user_to_tenant(tenant_id=tenant_id, inviter_id=auth_context.subject_id, email=invite_data.email, role=invite_data.role)
...
async def accept_invitation(acceptance_data: AcceptInvitation, registration_service: UserRegistrationService=Depends(get_registration_service)) -> Dict[str, Any]:
    try:
        registration_request = UserRegistrationRequest(username=acceptance_data.registration.username, email=acceptance_data.registration.email, password=acceptance_data.registration.password, first_name=acceptance_data.registration.first_name, last_name=acceptance_data.registration.last_name, company_name=acceptance_data.registration.company_name, plan=acceptance_data.registration.plan)
        result = await registration_service.accept_invitation(invite_token=acceptance_data.invite_token, registration=registration_request)
...
async def get_current_user(auth_context: AuthContext=Depends(get_auth_context), registration_service: UserRegistrationService=Depends(get_registration_service)) -> Dict[str, Any]:
    try:
        ...
        tenant_id = UUID(user.metadata.get('tenant_id'))
        tenant_info = await registration_service.get_tenant_info(tenant_id)
        return {'user': user.to_dict(), 'tenant': tenant_info, 'auth_context': auth_context.to_dict()}
    except Exception as e:
...
async def get_tenant_info(auth_context: AuthContext=Depends(get_auth_context), registration_service: UserRegistrationService=Depends(get_registration_service)) -> Dict[str, Any]:
    try:
        tenant_id = UUID(auth_context.metadata.get('tenant_id'))
        tenant_info = await registration_service.get_tenant_info(tenant_id)
        return tenant_info
...
async def list_tenant_users(auth_context: AuthContext=Depends(get_auth_context), registration_service: UserRegistrationService=Depends(get_registration_service)) -> Dict[str, Any]:
    try:
        if not auth_context.has_permission('user:manage'):
            user = await registration_service.jwt_provider.get_user_by_id(auth_context.subject_id)
            return {'users': [user.to_dict()] if user else [], 'total': 1 if user else 0, 'can_manage': False}
        ...
        tenant_users = [user for user in all_users if user.get('metadata', {}).get('tenant_id') == tenant_id]
        return {'users': tenant_users, 'total': len(tenant_users), 'can_manage': True}
    except Exception as e:
...
async def request_password_reset(reset_data: PasswordReset, registration_service: UserRegistrationService=Depends(get_registration_service)) -> Dict[str, str]:
    try:
        logger.info(f'Password reset requested for: {reset_data.email}')
        return {'status': 'success', 'message': 'If the email exists, you will receive reset instructions'}
    except Exception as e:
...
async def confirm_password_reset(reset_data: PasswordResetConfirm, registration_service: UserRegistrationService=Depends(get_registration_service)) -> Dict[str, str]:
    try:
...
async def get_auth_statistics(auth_context: AuthContext=Depends(get_auth_context), registration_service: UserRegistrationService=Depends(get_registration_service)) -> Dict[str, Any]:
    try:
        ...
        return {'jwt_provider': jwt_stats, 'tenants': {'total': tenant_count, 'active': active_tenants, 'by_plan': {'free': len([t for t in registration_service.tenants.values() if t.plan == 'free']), 'pro': len([t for t in registration_service.tenants.values() if t.plan == 'pro']), 'enterprise': len([t for t in registration_service.tenants.values() if t.plan == 'enterprise'])}}, 'pending_invitations': len(registration_service.pending_registrations)}
    except Exception as e:
