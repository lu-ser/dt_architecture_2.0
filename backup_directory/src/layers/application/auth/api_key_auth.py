...
from src.utils.exceptions import AuthenticationError, ValidationError, RateLimitError
...
class ApplicationType:
    ...
    EXTERNAL = 'external'
    TESTING = 'testing'
    TYPE_PERMISSIONS = {INTERNAL: ['digital_twin:read', 'digital_twin:write', 'digital_twin:execute', 'service:read', 'service:write', 'service:execute', 'replica:read', 'replica:write', 'replica:manage', 'workflow:read', 'workflow:write', 'workflow:execute', 'system:monitor'], PARTNER: ['digital_twin:read', 'digital_twin:execute', 'service:read', 'service:execute', 'replica:read', 'workflow:read', 'workflow:execute'], EXTERNAL: ['digital_twin:read', 'service:read', 'replica:read', 'workflow:read'], TESTING: ['digital_twin:read', 'digital_twin:write', 'service:read', 'service:execute', 'replica:read']}
...
class RateLimit:
    ...
    def __init__(self, requests_per_minute: int=60, requests_per_hour: int=1000, requests_per_day: int=10000, burst_limit: int=10):
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'requests_per_minute': self.requests_per_minute, 'requests_per_hour': self.requests_per_hour, 'requests_per_day': self.requests_per_day, 'burst_limit': self.burst_limit}

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'RateLimit':
        return cls(requests_per_minute=data.get('requests_per_minute', 60), requests_per_hour=data.get('requests_per_hour', 1000), requests_per_day=data.get('requests_per_day', 10000), burst_limit=data.get('burst_limit', 10))
...
class APIKeyUsage:
    ...
    def __init__(self, api_key_id: UUID):
        ...
        self.requests_by_minute: Dict[int, int] = {}
        self.requests_by_hour: Dict[int, int] = {}
        self.requests_by_day: Dict[int, int] = {}
    ...
    def get_current_usage(self) -> Dict[str, Any]:
        ...
        recent_cutoff = now - 60
        recent_count = len([req for req in self.recent_requests if req > recent_cutoff])
        return {'total_requests': self.total_requests, 'successful_requests': self.successful_requests, 'failed_requests': self.failed_requests, 'success_rate': self.successful_requests / max(self.total_requests, 1), 'last_used': self.last_used.isoformat() if self.last_used else None, 'first_used': self.first_used.isoformat() if self.first_used else None, 'current_usage': {'this_minute': self.requests_by_minute.get(minute_key, 0), 'this_hour': self.requests_by_hour.get(hour_key, 0), 'this_day': self.requests_by_day.get(day_key, 0), 'recent_burst': recent_count}}
    ...
    def _cleanup_old_data(self, current_timestamp: float) -> None:
        ...
        self.requests_by_minute = {k: v for k, v in self.requests_by_minute.items() if k >= current_minute - 5}
        self.requests_by_hour = {k: v for k, v in self.requests_by_hour.items() if k >= current_hour - 25}
        self.requests_by_day = {k: v for k, v in self.requests_by_day.items() if k >= current_day - 8}
        cutoff = current_timestamp - 120
        self.recent_requests = [req for req in self.recent_requests if req > cutoff]

class APIKey:

    def __init__(self, key_id: UUID, application_id: UUID, name: str, key_hash: str, permissions: List[str], rate_limit: RateLimit, is_active: bool=True, expires_at: Optional[datetime]=None, metadata: Optional[Dict[str, Any]]=None, created_at: Optional[datetime]=None):
        self.key_id = key_id
    ...
    def to_dict(self, include_sensitive: bool=False) -> Dict[str, Any]:
        key_dict = {'key_id': str(self.key_id), 'application_id': str(self.application_id), 'name': self.name, 'permissions': self.permissions, 'rate_limit': self.rate_limit.to_dict(), 'is_active': self.is_active, 'expires_at': self.expires_at.isoformat() if self.expires_at else None, 'metadata': self.metadata, 'created_at': self.created_at.isoformat(), 'usage': self.usage.get_current_usage()}
...
class Application:
    ...
    def __init__(self, application_id: UUID, name: str, application_type: str, description: str, contact_email: str, permissions: List[str], default_rate_limit: RateLimit, is_active: bool=True, metadata: Optional[Dict[str, Any]]=None, created_at: Optional[datetime]=None):
        self.application_id = application_id
        self.name = name
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'application_id': str(self.application_id), 'name': self.name, 'application_type': self.application_type, 'description': self.description, 'contact_email': self.contact_email, 'permissions': self.permissions, 'default_rate_limit': self.default_rate_limit.to_dict(), 'is_active': self.is_active, 'metadata': self.metadata, 'created_at': self.created_at.isoformat()}
...
class APIKeyProvider:
    ...
    def __init__(self):
        ...
        self.keys_by_app: Dict[UUID, List[UUID]] = {}
        self.rate_limit_violations: Dict[str, List[datetime]] = {}
        self.default_rate_limits = {ApplicationType.INTERNAL: RateLimit(120, 5000, 50000, 20), ApplicationType.PARTNER: RateLimit(60, 2000, 20000, 15), ApplicationType.EXTERNAL: RateLimit(30, 1000, 10000, 10), ApplicationType.TESTING: RateLimit(10, 100, 1000, 5)}
    ...
    async def validate_api_key(self, api_key: str) -> AuthContext:
        ...
        context = AuthContext(subject_type=AuthSubjectType.APPLICATION, subject_id=api_key_entity.application_id, auth_method=AuthMethod.API_KEY, permissions=api_key_entity.permissions, metadata={'application_name': application.name, 'application_type': application.application_type, 'api_key_name': api_key_entity.name, 'api_key_id': str(api_key_entity.key_id), 'contact_email': application.contact_email}, expires_at=api_key_entity.expires_at)
        return context

    async def create_application(self, name: str, application_type: str, description: str, contact_email: str, custom_permissions: Optional[List[str]]=None, custom_rate_limit: Optional[RateLimit]=None, metadata: Optional[Dict[str, Any]]=None) -> Application:
        ...
        rate_limit = custom_rate_limit or self.default_rate_limits.get(application_type, self.default_rate_limits[ApplicationType.EXTERNAL])
        application = Application(application_id=uuid4(), name=name, application_type=application_type, description=description, contact_email=contact_email, permissions=permissions, default_rate_limit=rate_limit, metadata=metadata or {})
    ...
    async def create_api_key(self, application_id: UUID, name: str, permissions: Optional[List[str]]=None, rate_limit: Optional[RateLimit]=None, expires_days: Optional[int]=None, metadata: Optional[Dict[str, Any]]=None) -> Tuple[str, APIKey]:
        ...
        api_key_entity = APIKey(key_id=uuid4(), application_id=application_id, name=name, key_hash=key_hash, permissions=permissions, rate_limit=rate_limit, expires_at=expires_at, metadata=metadata or {})
    ...
    async def rotate_api_key(self, old_api_key: str, name: Optional[str]=None) -> Tuple[str, APIKey]:
        ...
        new_api_key, new_key_entity = await self.create_api_key(application_id=old_key_entity.application_id, name=new_name, permissions=old_key_entity.permissions, rate_limit=old_key_entity.rate_limit, metadata={**old_key_entity.metadata, 'rotated_from': str(old_key_entity.key_id)})
    ...
    async def get_application_api_keys(self, application_id: UUID) -> List[APIKey]:
        return [api_key for api_key in self.api_keys.values() if api_key.application_id == application_id]
    ...
    async def list_applications(self, include_inactive: bool=False) -> List[Dict[str, Any]]:
        applications = []
        for app in self.applications.values():
    ...
    async def _create_default_applications(self) -> None:
        default_applications = [{'name': 'Internal Dashboard', 'application_type': ApplicationType.INTERNAL, 'description': 'Internal company dashboard application', 'contact_email': 'admin@company.com', 'metadata': {'default_app': True}}, {'name': 'Partner Integration', 'application_type': ApplicationType.PARTNER, 'description': 'Partner system integration', 'contact_email': 'partner@external.com', 'metadata': {'default_app': True}}, {'name': 'Mobile App', 'application_type': ApplicationType.EXTERNAL, 'description': 'Mobile application for external users', 'contact_email': 'mobile@company.com', 'metadata': {'default_app': True}}, {'name': 'Testing Suite', 'application_type': ApplicationType.TESTING, 'description': 'Automated testing and development', 'contact_email': 'dev@company.com', 'metadata': {'default_app': True}}]
        for app_data in default_applications:
            try:
                app = await self.create_application(**app_data)
                api_key, key_entity = await self.create_api_key(application_id=app.application_id, name=f'{app.name} Default Key', expires_days=365, metadata={'default_key': True})
    ...
    def _track_rate_limit_violation(self, key_hash: str) -> None:
        ...
        self.rate_limit_violations[key_hash] = [violation for violation in self.rate_limit_violations[key_hash] if violation > cutoff]
    ...
    def get_provider_status(self) -> Dict[str, Any]:
        ...
        return {'initialized': self._initialized, 'total_applications': len(self.applications), 'active_applications': len([app for app in self.applications.values() if app.is_active]), 'total_api_keys': total_keys, 'active_api_keys': active_keys, 'total_requests': total_requests, 'rate_limit_violations_24h': total_violations, 'application_types': app_type_dist, 'default_rate_limits': {app_type: limit.to_dict() for app_type, limit in self.default_rate_limits.items()}}