...
class User:
    ...
    def to_dict(self, include_sensitive: bool=False) -> Dict[str, Any]:
        user_dict = {'user_id': str(self.user_id), 'username': self.username, 'email': self.email, 'role': self.role, 'is_active': self.is_active, 'metadata': self.metadata, 'created_at': self.created_at.isoformat(), 'last_login': self.last_login.isoformat() if self.last_login else None}
...
class RefreshToken:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'token_id': str(self.token_id), 'user_id': str(self.user_id), 'token_hash': self.token_hash, 'expires_at': self.expires_at.isoformat(), 'is_revoked': self.is_revoked, 'metadata': self.metadata, 'created_at': self.created_at.isoformat()}
...
class JWTProvider:
    ...
    def get_provider_status(self) -> Dict[str, Any]:
        ...
        return {'initialized': self._initialized, 'total_users': len(self.users), 'active_users': len([u for u in self.users.values() if u.is_active]), 'active_refresh_tokens': active_tokens, 'total_refresh_tokens': len(self.refresh_tokens), 'active_sessions': sum((len(sessions) for sessions in self.active_sessions.values())), 'configuration': {'access_token_expire_minutes': self.access_token_expire_minutes, 'refresh_token_expire_days': self.refresh_token_expire_days, 'max_sessions_per_user': self.max_sessions_per_user, 'algorithm': self.algorithm}}