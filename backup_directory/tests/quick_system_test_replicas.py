...
def create_test_templates(temp_dir: Path):
    ...
    iot_template = {'template_id': 'quick_test_iot', 'name': 'Quick Test IoT Sensor', 'template_type': 'digital_replica', 'version': '1.0.0', 'description': 'Quick test IoT sensor replica', 'ontology_classes': ['TestDevice'], 'configuration': {'replica_type': 'sensor_aggregator', 'aggregation_mode': 'batch', 'aggregation_config': {'batch_size': 3, 'method': 'average'}, 'data_retention_policy': {'retention_days': 1}, 'quality_thresholds': {'min_quality': 0.5}}, 'metadata': {'allowed_override_keys': ['aggregation_config.batch_size'], 'required_keys': ['replica_type', 'aggregation_mode']}}
    with open(dr_templates_dir / 'quick_test_iot.json', 'w') as f:
        json.dump(iot_template, f, indent=2)
    test_ontology = {'name': 'QuickTest_Ontology', 'version': '1.0.0', 'namespace': 'http://quicktest.example/', 'classes': [{'name': 'TestDevice', 'uri': 'http://quicktest.example/#TestDevice', 'properties': [{'name': 'deviceId', 'property_type': 'datatype', 'range_type': 'string', 'constraints': {'required': True}}, {'name': 'temperature', 'property_type': 'datatype', 'range_type': 'float', 'constraints': {'min_value': -50, 'max_value': 100}}]}]}
...
async def test_system_workflow():
    ...
    try:
        ...
        try:
            ...
            from src.core.interfaces.replica import ReplicaType, DataAggregationMode, DeviceData, DataQuality, AggregatedData
        ...
        complete_config = {**base_config, 'parent_digital_twin_id': str(uuid4()), 'device_ids': ['test-device-001', 'test-device-002'], 'data_retention_policy': {'retention_days': 1}, 'quality_thresholds': {'min_quality': 0.5}}
        validation_errors = ontology_manager.validate_configuration({'deviceId': 'test-device-001', 'temperature': 25.0}, 'QuickTest_Ontology', 'TestDevice')
        ...
        association = DeviceAssociation(device_id='test-device-001', replica_id=replica.id, association_type='managed')
        ...
        device_data = [DeviceData(device_id='test-device-001', timestamp=datetime.now(timezone.utc), data={'temperature': 25.0, 'humidity': 60.0}, data_type='sensor', quality=DataQuality.HIGH), DeviceData(device_id='test-device-001', timestamp=datetime.now(timezone.utc), data={'temperature': 27.0, 'humidity': 65.0}, data_type='sensor', quality=DataQuality.HIGH), DeviceData(device_id='test-device-001', timestamp=datetime.now(timezone.utc), data={'temperature': 26.0, 'humidity': 62.0}, data_type='sensor', quality=DataQuality.HIGH)]
        ...
        target = ReplicaDeploymentTarget(target_id='test_target', target_type='container', capacity={'memory': 512, 'cpu': 1.0, 'max_replicas': 5}, location='test')
...
async def test_template_loading():
    ...
    try:
        ...
        config = manager.apply_template('quick_test_iot', {'aggregation_config.batch_size': 10})
...
async def test_data_aggregation():
    ...
    try:
        ...
        aggregator = StandardDataAggregator()
        test_data = [DeviceData(device_id='device-001', timestamp=datetime.now(timezone.utc), data={'temperature': 20.0, 'humidity': 50.0}, data_type='sensor', quality=DataQuality.HIGH), DeviceData(device_id='device-001', timestamp=datetime.now(timezone.utc), data={'temperature': 30.0, 'humidity': 70.0}, data_type='sensor', quality=DataQuality.HIGH), DeviceData(device_id='device-001', timestamp=datetime.now(timezone.utc), data={'temperature': 25.0, 'humidity': 60.0}, data_type='sensor', quality=DataQuality.HIGH)]
...
def check_system_health():
    ...
    components = {'Core Interfaces': 'src.core.interfaces.base', 'Replica Interfaces': 'src.core.interfaces.replica', 'Utils Config': 'src.utils.config', 'Utils Exceptions': 'src.utils.exceptions', 'Test Mocks': 'tests.mocks.storage_adapter', 'DR Factory': 'src.layers.virtualization.dr_factory', 'DR Registry': 'src.layers.virtualization.dr_registry', 'DR Management': 'src.layers.virtualization.dr_management', 'Ontology Manager': 'src.layers.virtualization.ontology.manager'}
    available = {}
    missing = []
...
async def main():
    ...
    all_core_available = all([available.get('DR Factory', False), available.get('DR Registry', False), available.get('DR Management', False), available.get('Ontology Manager', False)])
