    ...
    def _load_default_config(self) -> Dict[str, Any]:
        return {
            ...
            'mongodb': {
                ...
                'retry_writes': os.getenv('DT_MONGO_RETRY_WRITES', 'true').lower() == 'true',
                'auth_source': os.getenv('DT_MONGO_AUTH_SOURCE', 'admin'),
            ...
            'storage': {
                'primary_type': os.getenv('DT_STORAGE_PRIMARY', 'mongodb'),  # mongodb, postgresql, memory
                'cache_type': os.getenv('DT_STORAGE_CACHE', 'redis'),       # redis, memory, none
                'separate_dbs_per_twin': os.getenv('DT_SEPARATE_DBS', 'true').lower() == 'true',
                'auto_migration': os.getenv('DT_AUTO_MIGRATION', 'false').lower() == 'true'
            },
            
            # Legacy PostgreSQL (kept for compatibility)
            'database': {
