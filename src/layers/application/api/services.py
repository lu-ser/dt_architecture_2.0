"""
Services API endpoints for the Digital Twin Platform.

This module provides REST API endpoints for managing Services,
including CRUD operations, execution, discovery, and workflow management.

LOCATION: src/layers/application/api/services.py
"""
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "instance_name": "Production Analytics Service",
                "digital_twin_id": "123e4567-e89b-12d3-a456-426614174000",
                "parameters": {
                    "analytics_type": "basic_statistics",
                    "data_sources": ["sensor_001", "sensor_002"]
                },
                "execution_config": {
                    "timeout": 60,
                    "retry_count": 3
            ...
            "example": {
                "input_data": {
                    "data_points": [
                        ...
                        {"value": 15, "timestamp": "2024-01-01T10:05:00Z"}
                    ]
                },
...
class WorkflowCreate(BaseModel):
    """Model for creating a Service Workflow."""
    ...
    class Config:
        schema_extra = {
            "example": {
                "workflow_name": "Analytics and Prediction Pipeline",
                "digital_twin_id": "123e4567-e89b-12d3-a456-426614174000",
                "service_chain": [
                    {
...
async def list_services(
    ...
    digital_twin_id: Optional[UUID] = Query(None, description="Filter by Digital Twin"),
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Failed to list Services: {e}"
        )
    ...
    try:
        ...
        if not service_data.definition_id and not service_data.template_id:
            raise HTTPException(
                ...
                detail="Either definition_id or template_id must be provided"
            )
        
        if service_data.definition_id and service_data.template_id:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="Cannot specify both definition_id and template_id"
            )
        
        # Convert Pydantic model to dict
        service_config = service_data.dict()
        
        # Create Service via gateway
...
async def get_service(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get Service: {e}"
        )
...
async def delete_service(
    ...
    try:
        ...
        return {
            "message": f"Service {service_id} deletion requested",
            "status": "accepted"
        }
    except EntityNotFoundError:
        raise HTTPException(
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to delete Service: {e}"
        )
...
async def execute_service(
    service_id: UUID = Path(..., description="Service ID"),
    execution_data: ServiceExecution = Body(...),
    gateway: APIGateway = Depends(get_gateway)
) -> Dict[str, Any]:
    """Execute a Service with the provided data."""
    try:
        result = await gateway.execute_service(
            service_id=service_id,
            input_data=execution_data.input_data,
            execution_parameters=execution_data.execution_parameters,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Service execution failed: {e}"
        )
...
async def execute_analytics(
    service_id: UUID = Path(..., description="Service ID"),
    data_points: List[Dict[str, Any]] = Body(..., description="Data points to analyze"),
    analytics_config: Optional[Dict[str, Any]] = Body(None, description="Analytics configuration"),
    gateway: APIGateway = Depends(get_gateway)
) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Analytics execution failed: {e}"
        )
...
async def execute_prediction(
    service_id: UUID = Path(..., description="Service ID"),
    historical_data: List[Dict[str, Any]] = Body(..., description="Historical data for prediction"),
    prediction_horizon: int = Body(300, ge=1, description="Prediction horizon in seconds"),
    model_config: Optional[Dict[str, Any]] = Body(None, description="Model configuration"),
    gateway: APIGateway = Depends(get_gateway)
) -> Dict[str, Any]:
    """Execute prediction based on historical data."""
    try:
        execution_data = {
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Prediction execution failed: {e}"
        )
...
async def create_workflow(
    ...
    try:
        # Note: This would be implemented in the service orchestrator
        # For now, return a placeholder response
        ...
        return {
            ...
            "digital_twin_id": str(workflow_data.digital_twin_id),
            "service_chain_length": len(workflow_data.service_chain),
            "status": "created",
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Workflow creation failed: {e}"
        )
...
async def execute_workflow(
    ...
    input_data: Dict[str, Any] = Body(..., description="Input data for workflow"),
    execution_config: Optional[Dict[str, Any]] = Body(None, description="Execution configuration"),
    gateway: APIGateway = Depends(get_gateway)
) -> Dict[str, Any]:
    """Execute a Service Workflow."""
    try:
        # Note: This would be implemented via the gateway
...
async def get_workflow_status(
    ...
    """Get the status of a Service Workflow."""
    try:
        # Note: This would be implemented via the gateway
        return {
            "workflow_id": workflow_id,
...
async def get_service_status(
    ...
    """Get detailed status and health information for a Service."""
    try:
        ...
        status_info = {
            ...
            "health": "healthy",  # Would be from health check
            "performance": {
                "success_rate": 0.95,  # Would be from metrics
...
async def get_service_metrics(
    ...
    try:
        # Note: This would get real metrics from the service layer
        return {
            "service_id": str(service_id),
            "time_range_seconds": time_range,
            "metrics": {
                "total_executions": 1247,
                "successful_executions": 1182,
                "failed_executions": 65,
                "success_rate": 0.948,
                "average_execution_time": 0.145,
                "min_execution_time": 0.023,
                "max_execution_time": 2.156,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get service metrics: {e}"
        )
...
async def get_available_definitions(
    service_type: Optional[str] = Query(None, description="Filter by service type"),
    gateway: APIGateway = Depends(get_gateway)
) -> Dict[str, Any]:
    """Get available Service definitions."""
    try:
        # Convert service_type to enum if provided
        type_filter = None
        if service_type:
            try:
                ...
                raise HTTPException(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    detail=f"Invalid service_type. Must be one of: {[t.value for t in ServiceType]}"
                )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get service definitions: {e}"
        )
...
async def bulk_execute_services(
    service_executions: List[Dict[str, Any]] = Body(..., description="List of service executions"),
    gateway: APIGateway = Depends(get_gateway)
) -> Dict[str, Any]:
    """Execute multiple services in batch."""
    try:
        results = []
        ...
        for execution in service_executions:
            try:
                ...
                results.append({
                    "service_id": execution.get("service_id", "unknown"),
                    "success": False,
                    "error": str(e)
                })
        ...
        return {
            ...
            "failed": len(service_executions) - successful,
            "results": results,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Bulk execution failed: {e}"
        )