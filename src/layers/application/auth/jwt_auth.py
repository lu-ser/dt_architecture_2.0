...
async def demo_jwt_authentication():
    """Demonstrate JWT authentication capabilities."""
    print("🔐 Digital Twin Platform - JWT Authentication Demo")
    print("=" * 60)
    
    # Initialize JWT provider
    jwt_provider = JWTProvider()
    await jwt_provider.initialize()
    
    print(f"✅ JWT Provider initialized")
    print(f"📊 Status: {jwt_provider.get_provider_status()}")
    ...
    try:
        ...
        auth_context = await jwt_provider.authenticate({
            "username": "admin",
            "password": "admin123"
        })
        print(f"✅ Admin authenticated: {auth_context.metadata['username']} ({auth_context.metadata['role']})")
        
        # Generate token pair
        admin_user = await jwt_provider.get_user_by_username("admin")
        token_pair = await jwt_provider.generate_token_pair(admin_user)
        print(f"✅ Token pair generated: {len(token_pair.access_token)} chars access token")
        
        # Validate token
        validated_context = await jwt_provider.validate_token(token_pair.access_token)
        print(f"✅ Token validated: {validated_context.metadata['username']}")
        
        # Test refresh token
        new_token_pair = await jwt_provider.refresh_token(token_pair.refresh_token)
        print(f"✅ Token refreshed: New access token generated")
        
        # Test different user roles
        print(f"\n👥 Testing different user roles...")
        
        for username in ["admin", "operator", "viewer"]:
            user = await jwt_provider.get_user_by_username(username)
            permissions = user.get_permissions()
            print(f"   • {user.role.upper()}: {len(permissions)} permissions")
            
        # Test permission checking
        print(f"\n🔍 Testing permission checks...")
        admin_context = await jwt_provider.authenticate({"username": "admin", "password": "admin123"})
        viewer_context = await jwt_provider.authenticate({"username": "viewer", "password": "viewer123"})
        
        admin_can_write = admin_context.has_permission("digital_twin:write")
        viewer_can_write = viewer_context.has_permission("digital_twin:write")
        viewer_can_read = viewer_context.has_permission("digital_twin:read")
        
        print(f"   • Admin can write DT: {admin_can_write}")
        print(f"   • Viewer can write DT: {viewer_can_write}")
        print(f"   • Viewer can read DT: {viewer_can_read}")
...
if __name__ == "__main__":
    asyncio.run(demo_jwt_authentication())