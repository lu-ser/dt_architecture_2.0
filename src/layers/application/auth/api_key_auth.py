...
async def demo_api_key_authentication():
    """Demonstrate API key authentication capabilities."""
    print("🔑 Digital Twin Platform - API Key Authentication Demo")
    print("=" * 60)
    ...
    print(f"✅ API Key Provider initialized")
    print(f"📊 Status: {api_provider.get_provider_status()}")
    
    try:
        # List default applications
        applications = await api_provider.list_applications()
        print(f"\n📱 Default applications created:")
        for app in applications:
            print(f"   • {app['name']} ({app['application_type']}) - {app['api_key_count']} keys")
        
        # Get a default API key for testing
        internal_app = next(app for app in applications if app['name'] == 'Internal Dashboard')
        api_key = internal_app['metadata']['default_api_key']
        
        print(f"\n🧪 Testing API key authentication...")
        print(f"Using API key: {api_key[:20]}...")
        
        # Test authentication
        auth_context = await api_provider.authenticate({"api_key": api_key})
        print(f"✅ Application authenticated: {auth_context.metadata['application_name']}")
        print(f"   • Type: {auth_context.metadata['application_type']}")
        print(f"   • Permissions: {len(auth_context.permissions)}")
        
        # Test multiple requests to show rate limiting tracking
        print(f"\n⚡ Testing rate limiting...")
        for i in range(5):
            context = await api_provider.validate_api_key(api_key)
            usage = await api_provider.get_api_key_usage(api_key)
            print(f"   • Request {i+1}: {usage['current_usage']['this_minute']} requests this minute")
        
        # Test different application types
        print(f"\n🔍 Testing different application types...")
        
        for app in applications[:3]:  # Test first 3 apps
            app_api_key = app['metadata']['default_api_key']
            context = await api_provider.authenticate({"api_key": app_api_key})
            
            can_write = context.has_permission("digital_twin:write")
            can_execute = context.has_permission("service:execute")
            
            print(f"   • {app['application_type'].upper()}: write={can_write}, execute={can_execute}")
        
        # Test API key rotation
        print(f"\n🔄 Testing API key rotation...")
        new_key, new_entity = await api_provider.rotate_api_key(api_key, "Rotated Test Key")
        print(f"✅ API key rotated: {new_entity.name}")
        print(f"   • New key: {new_key[:20]}...")
        
        # Test new key works
        new_context = await api_provider.validate_api_key(new_key)
        print(f"✅ New key works: {new_context.metadata['api_key_name']}")
        
        print(f"\n✅ API Key Authentication system working perfectly!")
...
if __name__ == "__main__":
    asyncio.run(demo_api_key_authentication())