...
async def demo_service_layer():
    """Demonstrate the service layer capabilities."""
    print("🚀 Digital Twin Platform - Service Layer Demo")
    print("=" * 60)
    
    # Initialize the layer
    orchestrator = await initialize_service_layer()
    await orchestrator.start()
    
    print(f"✅ Service Layer initialized")
    
    # Show available service definitions
    definitions = await orchestrator.get_available_service_definitions()
    print(f"📋 Available service definitions: {len(definitions)}")
    for definition in definitions:
        print(f"   • {definition['name']} ({definition['definition_id']})")
    
    # Show available templates if enabled
    if orchestrator.templates_integration:
        templates = await orchestrator.get_service_templates()
        print(f"📄 Available service templates: {len(templates)}")
    ...
    analytics_service = await create_analytics_service(
        digital_twin_id=dt_id,
        data_sources=["sensor-001", "sensor-002"],
        analytics_type="basic_statistics"
    )
    print(f"✅ Created analytics service: {analytics_service.id}")
    
    # Create prediction service
    prediction_service = await create_prediction_service(
        digital_twin_id=dt_id,
        model_type="linear",
        prediction_horizon=600
    )
    print(f"✅ Created prediction service: {prediction_service.id}")
    
    # Execute analytics service
    print(f"\n⚙️ Executing Services...")
    
    analytics_response = await orchestrator.execute_service(
        service_id=analytics_service.id,
        input_data={
            "data_points": [
                {"value": 10, "timestamp": "2024-01-01T10:00:00Z"},
                {"value": 15, "timestamp": "2024-01-01T10:05:00Z"},
                {"value": 12, "timestamp": "2024-01-01T10:10:00Z"}
            ]
        }
    )
    print(f"✅ Analytics service executed: {analytics_response.success}")
    
    # Create and execute workflow
    print(f"\n🔄 Creating Service Workflow...")
    
    workflow_id = await orchestrator.create_service_workflow(
        workflow_name="Analytics and Prediction Pipeline",
        service_chain=[
            {
                "step_name": "analyze_data",
                "service_id": str(analytics_service.id),
                "config": {}
            },
            {
                "step_name": "predict_trends",
                "service_id": str(prediction_service.id),
    ...
    execution_id = await orchestrator.execute_workflow(
        ...
        input_data={
            "data_points": [
                {"value": 20, "timestamp": "2024-01-01T11:00:00Z"},
                {"value": 25, "timestamp": "2024-01-01T11:05:00Z"}
            ]
        }
    )
    ...
    stats = await orchestrator.get_layer_statistics()
    print(f"   • Total services: {stats['registry']['total_services']}")
    print(f"   • Service definitions: {stats['factory']['available_definitions']}")
    print(f"   • Active workflows: {stats['workflows']['active_workflows']}")
    print(f"   • Total executions: {stats['registry']['performance']['total_executions']}")
...
if __name__ == "__main__":
    asyncio.run(demo_service_layer())