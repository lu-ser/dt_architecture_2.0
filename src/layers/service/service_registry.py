"""
Service Registry implementation for the Digital Twin Platform.

This module provides the specialized registry for Service instances,
including service discovery, performance tracking, and capability-based queries.
"""
...
class ServiceAssociation:
    """Represents the association between a service and a Digital Twin."""
    
    def __init__(
        self,
        service_id: UUID,
        digital_twin_id: UUID,
        association_type: str = "provides_capability",
        created_at: Optional[datetime] = None,
        metadata: Optional[Dict[str, Any]] = None
    ):
        self.service_id = service_id
        self.digital_twin_id = digital_twin_id
        self.association_type = association_type  # provides_capability, processes_data, monitors
    ...
    def update_execution_stats(self, success: bool, execution_time: float) -> None:
        """Update statistics when service executes."""
...
class ServicePerformanceMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "executions_by_type": {stype.value: count for stype, count in self.executions_by_type.items()},
            "executions_by_priority": {priority.value: count for priority, count in self.executions_by_priority.items()},
            "response_time_percentiles": {
                "p50": self.get_percentile_response_time(50),
                "p95": self.get_percentile_response_time(95),
                "p99": self.get_percentile_response_time(99)
            }
        }


class ServiceMetrics(RegistryMetrics):
    """Extended metrics for Service Registry."""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        base_metrics.update({
            ...
            "services_by_state": self.services_by_state,
...
class ServiceRegistry(AbstractRegistry[IService]):
    """
    Registry for Service entities.
    
    Provides specialized functionality for Services including
    capability-based discovery, performance tracking, and association management.
    """
    
    def __init__(
        self,
        storage_adapter: IStorageAdapter[IService],
        cache_enabled: bool = True,
        cache_size: int = 1000,
        cache_ttl: int = 300
    ):
        super().__init__(
            entity_type=IService,
        ...
        # Override metrics with service-specific metrics
    ...
    async def register_service(
        self,
        service: IService,
        associations: Optional[List[ServiceAssociation]] = None
    ) -> None:
        """
        Register a Service with optional associations.
        
        Args:
            service: Service to register
            associations: Optional service associations
        """
        # Register the service using base functionality
        ...
        if associations:
            ...
            # Create default association with parent Digital Twin
    ...
    async def find_services_by_digital_twin(self, digital_twin_id: UUID) -> List[IService]:
        """
        Find Services associated with a specific Digital Twin.
    ...
    async def find_services_by_state(self, state: ServiceState) -> List[IService]:
        ...
        """
        # This would require querying the actual service instances
        # For now, get all services and filter
    ...
    async def associate_service(self, association: ServiceAssociation) -> None:
        """
        Associate a service with a Digital Twin.
        
        Args:
            association: Service association to add
        """
        async with self._association_lock:
            ...
            # Update digital twin to services mapping
            ...
            self.logger.info(
                f"Associated service {association.service_id} "
                f"with Digital Twin {association.digital_twin_id}"
            )
    ...
    async def disassociate_service(
        ...
        """
        Remove association between a service and Digital Twin.
    ...
    async def get_service_associations(self, service_id: UUID) -> List[ServiceAssociation]:
        """
        Get all associations for a specific service.
        
        Args:
            service_id: ID of the service
            
        Returns:
            List of service associations
        """
        associations = []
    ...
    async def record_service_execution(
        self,
        service_id: UUID,
        execution_time: float,
        success: bool,
        ...
        """
        Record that a service execution occurred.
        
        Args:
            service_id: ID of the service that executed
            execution_time: Time taken for execution in seconds
            success: Whether the execution was successful
            priority: Priority of the execution
        """
        async with self._performance_lock:
            # Get service for type information
            try:
                service = await self.get_service(service_id)
            ...
            self.metrics.performance_metrics.record_execution(
                service_id, service_type, priority, execution_time, success
            )
    ...
    async def discover_services(
        ...
        Args:
            criteria: Discovery criteria (type, capabilities, state, etc.)
        ...
        if "min_success_rate" in criteria:
            ...
            for service in services:
                ...
                if associations:
                    ...
                    # No execution history, assume good
                    filtered_services.append(service)
    ...
    async def get_service_statistics(self) -> Dict[str, Any]:
        """
        Get comprehensive statistics about registered Services.
        ...
        stats = {
            ...
            "services_by_type": {},
            "services_by_state": {},
            "services_with_associations": len(set(assoc.service_id for assoc in self.service_associations.values())),
            "total_associations": len(self.service_associations),
            "total_capabilities": len(self.capability_services),
            "digital_twins_with_services": len(self.digital_twin_services)
        }
    ...
    async def _unregister_service_capabilities(self, service: IService) -> None:
        """Unregister service capabilities."""
        for capability in service.capabilities:
