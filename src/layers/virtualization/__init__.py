...
# Example usage
async def demo_virtualization_layer():
    """Demonstrate the virtualization layer capabilities."""
    from datetime import datetime, timezone
    
    print("🚀 Digital Twin Platform - Virtualization Layer Demo")
    print("=" * 60)
    
    # Initialize the layer
    orchestrator = await initialize_virtualization_layer()
    await orchestrator.start()
    
    print(f"✅ Virtualization Layer initialized")
    ...
    ontologies = await orchestrator.get_available_ontologies()
    ...
    # Create IoT sensor replica
    dt_id = uuid4()
    iot_replica = await create_iot_sensor_replica(
        parent_digital_twin_id=dt_id,
        device_ids=["sensor-001", "sensor-002", "sensor-003"],
        batch_size=5,
        quality_threshold=0.8
    )
    print(f"✅ Created IoT sensor replica: {iot_replica.id}")
    
    # Create industrial device replica
    industrial_replica = await create_industrial_device_replica(
        parent_digital_twin_id=dt_id,
        device_id="machine-001",
        real_time_required=True,
        high_security=True
    )
    print(f"✅ Created industrial device replica: {industrial_replica.id}")
    
    # Deploy as containers
    print(f"\n📦 Deploying replicas as containers...")
    container1 = await orchestrator.deploy_replica_as_container(
        iot_replica.id, "local_docker"
    )
    print(f"✅ Deployed {iot_replica.id} as container {container1.container_id}")
    
    # Show statistics
    print(f"\n📊 Virtualization Layer Statistics:")
    stats = await orchestrator.get_layer_statistics()
    print(f"   • Ontologies: {stats['ontology']['ontologies']['count']}")
    print(f"   • Templates: {stats['ontology']['templates']['count']}")
    print(f"   • Active replicas: {stats['registry']['device_count']}")
    print(f"   • Active containers: {stats['containers']['active_containers']}")
...
if __name__ == "__main__":
    asyncio.run(demo_virtualization_layer())