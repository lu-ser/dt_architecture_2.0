"""
Ontology and Template Management System for the Digital Twin Platform.

This module provides a generic system for loading, managing and applying
ontologies and templates from external sources.

LOCATION: src/layers/virtualization/ontology/manager.py
"""
...
class SemanticProperty:
    """Represents a semantic property from an ontology."""
    ...
    def __init__(
        ...
        self.name = name
        self.property_type = property_type  # datatype, object, annotation
        self.domain = domain
...
class SemanticClass:
    """Represents a semantic class from an ontology."""
    ...
    def __init__(
        ...
        properties: Optional[List[SemanticProperty]] = None,
        description: Optional[str] = None,
...
class Ontology:
    ...
    def _check_type_compatibility(self, value: Any, expected_type: str) -> bool:
        ...
        type_mapping = {
            ...
            "float": float,
            "boolean": bool,
            "datetime": str,  # We'll accept ISO strings
            "uri": str
        }
...
class Template:
    ...
    def __init__(
        ...
        ontology_classes: Optional[List[str]] = None,
        source_path: Optional[str] = None,
        metadata: Optional[Dict[str, Any]] = None
    ):
    ...
    def apply_overrides(self, overrides: Dict[str, Any]) -> Dict[str, Any]:
        """Apply configuration overrides to the template."""
    ...
    def validate_overrides(self, overrides: Dict[str, Any]) -> List[str]:
        """Validate configuration overrides against the template."""
        errors = []
        
        # Check for unknown keys (if template defines allowed keys)
        ...
        # Validate required keys are not removed
...
class IOntologyLoader(ABC):
    """Abstract interface for ontology loaders."""
...
class JSONOntologyLoader(IOntologyLoader):
    """Loader for JSON format ontologies."""
    ...
    async def load(self, file_path: Path) -> Ontology:
        ...
        try:
            ...
            ontology = Ontology(
                ...
                namespace=data.get("namespace", f"urn:{file_path.stem}"),
                format_type=OntologyFormat.JSON,
            ...
            for class_data in data.get("classes", []):
                ...
                for prop_data in class_data.get("properties", []):
                    prop = SemanticProperty(
                        name=prop_data["name"],
                        property_type=prop_data.get("property_type", "datatype"),
                        domain=prop_data.get("domain"),
                        range_type=prop_data.get("range_type"),
                ...
                semantic_class = SemanticClass(
                    ...
                    uri=class_data.get("uri", f"{ontology.namespace}#{class_data['name']}"),
...
class YAMLOntologyLoader(IOntologyLoader):
    """Loader for YAML format ontologies."""
    ...
    async def load(self, file_path: Path) -> Ontology:
        ...
        try:
            ...
            # Convert to JSON format and use JSON loader
            ...
            ontology = Ontology(
                ...
                format_type=OntologyFormat.YAML,
                source_path=str(file_path)
            )
            
            # Use same loading logic as JSON
...
class OntologyManager:
    """Manages ontologies and templates for the Digital Twin Platform."""
    ...
    def __init__(self, base_path: Optional[Path] = None):
        ...
        # Register loaders
        self.loaders: List[IOntologyLoader] = [
            JSONOntologyLoader(),
            YAMLOntologyLoader()
        ]
    ...
    async def load_ontologies(self, ontology_dir: Optional[Path] = None) -> None:
        """Load all ontologies from the specified directory."""
    ...
    def create_template_from_configuration(
        self,
        template_id: str,
        name: str,
        configuration: Dict[str, Any],
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for name, ontology in self.ontologies.items():
            ontology_stats[name] = {
                ...
                "format": ontology.format_type.value,
                "version": ontology.version
            }
        
        template_stats = {}
        for template_type in TemplateType:
            template_stats[template_type.value] = len([
        ...
        return {
            "ontologies": {
                "count": len(self.ontologies),
                "details": ontology_stats
            },
            "templates": {
                "count": len(self.templates),
                "by_type": template_stats
            }
        }
...
async def initialize_ontology_system(base_path: Optional[Path] = None) -> OntologyManager:
    """Initialize the ontology system with loading of all ontologies and templates."""
    global _ontology_manager
