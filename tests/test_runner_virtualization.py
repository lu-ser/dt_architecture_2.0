#!/usr/bin/env python3
"""
Test Runner for Virtualization Layer Components

This script runs focused tests on the virtualization layer components
to verify they work correctly.

LOCATION: tests/test_runner_virtualization.py

Usage:
    python tests/test_runner_virtualization.py
    python tests/test_runner_virtualization.py --component factory
    python tests/test_runner_virtualization.py --integration
"""
...
def check_imports():
    ...
    # Try importing virtualization components (these may not exist yet)
    virtualization_components = {
        'Factory': 'src.layers.virtualization.dr_factory',
        'Registry': 'src.layers.virtualization.dr_registry', 
        'Management': 'src.layers.virtualization.dr_management',
        'Container': 'src.layers.virtualization.dr_container',
        'Ontology': 'src.layers.virtualization.ontology.manager'
    }
...
async def test_factory_component():
    """Test the Digital Replica Factory component."""
...
async def test_registry_component():
    """Test the Digital Replica Registry component."""
    ...
    try:
        ...
        # Test 2: Create and register a replica
        factory = DigitalReplicaFactory()
        config = {
            "replica_type": "sensor_aggregator",
            "parent_digital_twin_id": str(uuid4()),
...
async def test_ontology_component():
    """Test the Ontology and Template Management System."""
    print("\n🧠 Testing Ontology & Template System...")
    
    try:
        from src.layers.virtualization.ontology.manager import (
            OntologyManager, Template, TemplateType, JSONOntologyLoader
        )
        
        # Test 1: Create temporary directory with test files
        temp_dir = Path(tempfile.mkdtemp())
        
        try:
            # Test 2: Create ontology manager
            ...
            template = Template(
                ...
                configuration={
                    "replica_type": "sensor_aggregator",
                    "aggregation_mode": "batch",
                    "aggregation_config": {"batch_size": 10}
                },
                metadata={
            ...
            # Test 4: Apply template with overrides
            ...
            # Test 5: Test JSON ontology loading
            ontology_data = {
                "name": "Test_Ontology",
                "version": "1.0.0",
                "namespace": "http://test.example/",
                "classes": [
                    {
                        "name": "TestDevice",
                        "uri": "http://test.example/#TestDevice",
                        "properties": [
                            {
                                "name": "deviceId",
            ...
            invalid_config = {}  # Missing required deviceId
...
async def test_integration_workflow():
    """Test integration workflow combining multiple components."""
    ...
    try:
        ...
        # Step 1: Setup ontology manager with test template
        temp_dir = Path(tempfile.mkdtemp())
        ...
        try:
            ...
            # Step 2: Setup factory, registry, and lifecycle manager
            factory = DigitalReplicaFactory()
            storage_adapter = InMemoryStorageAdapter()
            registry = DigitalReplicaRegistry(storage_adapter)
            await registry.connect()
            ...
            # Step 3: Create replica from template
...
async def run_all_tests():
    ...
    # Run integration test if all core components are available
    core_available = all([
        available_components.get('Factory', False),
        available_components.get('Registry', False),
        available_components.get('Management', False),
        available_components.get('Ontology', False)
    ])
...
def main():
    ...
    parser.add_argument("--component", choices=["factory", "registry", "ontology", "integration"],
